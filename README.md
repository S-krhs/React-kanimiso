...
### Viteまわり
最初に調べて参考にしたプロジェクトがViteで作られていたため何も分からない状態で`create vite`を使っていた。検索で出てくるものは大抵`create-react-app`で作られているため`package.json`や`vite.config.js`まわりの書き方で躓いた。例えばproxyの設定とか。通常のnpmコマンドのデフォルト設定との違いもあり`npm run dev`とか`npm run start`とかで書き間違えが出たりするのも注意。（最初にまとめてコマンドを設定したほうが楽でいいのかもしれない）
