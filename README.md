...
実際コードを書くようになると、プログラムがどのように動いているのかというのを考えないといけない場面が多く出てきます。JavaScriptはブラウザ側の処理なのかサーバ側での処理なのかを意識する、というか常にそれを前提に考えておくのが必要だなと思います。今回はフロントでReact.js、バックでNode.jsを使ったので、基本的な書き方とフレームワーク特有の書き方両方に慣れることができたのは良かったかなと思います。<br>
